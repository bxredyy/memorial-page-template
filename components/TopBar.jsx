    ...
    >
      <div className="h-full max-w-[1500px] mx-auto px-8 flex items-center justify-between">
          ...
          {NAV_ITEMS.map((item, i) => (
            ...
            >
              <span className="relative">
                {item.label}
                <span
                  className="absolute -bottom-1 left-0 right-0 h-px bg-sage-500 origin-center scale-x-0 group-hover:scale-x-100 transition-transform duration-300 transform-gpu"
        ...
        <div className="flex items-center gap-4">
          <button
            className="md:hidden inline-flex items-center justify-center p-3 rounded-md bg-white/90 hover:bg-white transition shadow-sm"
      ...
      {menuOpen && (
        <div className="md:hidden fixed inset-0 z-50 bg-black/40 backdrop-blur-sm" onClick={() => setMenuOpen(false)}>
          <div
            className="absolute top-14 right-4 left-4 bg-cream-100 rounded-lg shadow-xl p-4 transform-gpu transition duration-200 ease-out"
            ...
            <nav className="flex flex-col gap-2">
              {NAV_ITEMS.map((item) => (
                <a
                  key={item.id}
                  href={`#${item.id}`}
                  className="block px-4 py-3 rounded-md text-ink-800 font-semibold tracking-wide uppercase text-center"
