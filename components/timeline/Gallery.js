...
class Gallery {
  ...
  async init(scene, containerElement = null) {
  ...
  setPlanes(scene) {
    ...
    this.planeConfig.forEach((plane, index) => {
      const texture = this.texturesBySource.get(plane.imageSrc) || null
      const textureImage = texture?.image
      const aspectRatio =
        textureImage && textureImage.width > 0 && textureImage.height > 0
          ? textureImage.width / textureImage.height
          : 0.78
      const fallbackColor = plane.accentColor || '#ffffff'
  ...
  getTextureSources() {
    const textureSources = this.planeConfig
      .map((planeDefinition) => planeDefinition.imageSrc)
      .filter(Boolean)
  ...
  bindPointerEvents() {
    const target = this.containerElement || window
